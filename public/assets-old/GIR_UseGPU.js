...
export function GIRCurve(insulinKG) {
    ...
    const getIntermediateYData = gpu.createKernel(function(intermediateXData, insulinKG) {
      const x = intermediateXData[this.thread.x];
      const y = 0.0033820425120803 * Math.pow(x, 5) - 0.0962642502970792 * Math.pow(x, 4) + 1.0161233494860400 * Math.pow(x, 3) -
      ...
      return intermediateY || 0; // Add a default value here
    }).setOutput([960, 2])
    .setPipeline(true);
...
const getSmallYData = gpu.createKernel(function(smallXData) {
  const x = smallXData[this.thread.x];
  const y = 0.0033820425120803 * Math.pow(x, 5) - 0.0962642502970792 * Math.pow(x, 4) + 1.0161233494860400 * Math.pow(x, 3) -
      4.7280409167367000 * Math.pow(x, 2) + 8.2811624637053000 * x - 0.4658832073238300;
  return y;
}).setOutput([1920]);
...
const getMediumYData = gpu.createKernel(function(mediumXData) {
  const x = mediumXData[this.thread.x];
  const y = 0.0004449113905105 * Math.pow(x, 6) - 0.0097881251143144 * Math.pow(x, 5) + 0.0487062677027909 * Math.pow(x, 4) +
      0.3395509285035820 * Math.pow(x, 3) - 3.8635372657493500 * Math.pow(x, 2) + 9.8215306047782600 * x - 0.5016675029655920;
  return y;
}).setOutput([1920]);


const getLargeYData = gpu.createKernel(function(largeXData) {
  const x = largeXData[this.thread.x];
  const y = -0.0224550824431891 * Math.pow(x, 4) + 0.5324819868175370 * Math.pow(x, 3) - 4.2740977490209200 * Math.pow(x, 2) +
      11.6354217632198000 * x - 0.0653457810255797;
  return y;
}).setOutput([1920]);
