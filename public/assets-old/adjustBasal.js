...
export async function adjustBasalRatesUsingTemps(averageCombinedData) {
...
export async function adjustBasalRatesUsingProfileBasals(averageCombinedData) {
    // console.log('averageCombinedData', averageCombinedData)
    const isf = averageCombinedData[0].isf;
    const lowTargetBG = options.lowTargetBG;
    const targetBG = options.targetBG;
    ...
    const predictedBGs = averageCombinedData.map((data) => data.bg);
    const outcomeBGs = averageCombinedData.map((data) => data.bg);
    let startingBasals = averageCombinedData.map((data) => data.profileBasal / 12);
    let startingBasalsPlusBolus = averageCombinedData.map((data) => ((data.profileBasal / 12) + (data.bolusInsulin / 12)));
    let estimatedBasal = averageCombinedData.map((data) => data.profileBasal / 12);
    ...
    async function runCalcs() {
        ...
        logs()
        const totalAdditionalInsulin = estimatedBasal.reduce((sum, rate, index) => sum + rate - averageCombinedData[index].profileBasal, 0);
